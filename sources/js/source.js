...
const motsInterdits = [ // Fichiers de mots interdits à entrer
    '<script>',
...
//* Code pour voir les données entrées par l'user
let input1 = document.getElementById("email")
input1.addEventListener("input", (event) => {
    const pattern = newRegExp ("[a-zA-Z]{5,100}")
...
input2.addEventListener("input", (event) => {
    for (const word of motsInterdits){
        if (mdp.value == word){mdp.value = ""} //* Reset la valeur du champ si un mot "motsInterdits" est entré par l'user
    }
    // Fonction de rappel = "collback"
    console.log( event.target.value);
});
